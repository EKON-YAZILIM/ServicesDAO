...
namespace DAO_WebPortal.Utility
{
    public static class Captcha
    {
        ...
        public static bool ValidateCaptchaCode(string code,string userInputCaptcha, HttpContext context)
        {
            if (string.IsNullOrEmpty(userInputCaptcha)) return false;

            var contextcaptcha = context.Session.GetString(code);
            bool isValid = true;
            if(userInputCaptcha == contextcaptcha)
            {
                isValid = true;
            }
            else
